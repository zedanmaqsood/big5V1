$(function () {
	...
	function scroll2Page(){
		...
		if(submitState && $warning){
			$body.animate({
            	scrollTop: $warning.first().offset().top}, 500);
        }
		else{
			$body.animate({
	...
	function loadPage(pageNumber) {
		...
		start = parseInt(pages[pageNumber].start);
		stop = parseInt(pages[pageNumber].finish);
		htmlCode = "";
	...
	$(".buttons").on('click tap',function(e){
		e.preventDefault();
		var unAns;
		var $target = $(e.target);
		...
		if($parentId=='submit'){
			...
			if(unAns){
				currentPageNumber = pageFromQNum(unAns[0]);
				if(currentPageNumber!=4){
					$("#submit").remove();
					$('#previous').after(nextBtn);
				}
